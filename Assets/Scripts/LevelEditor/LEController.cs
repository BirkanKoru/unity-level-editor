...
#if UNITY_EDITOR
/// <summary>
/// Level Editor Controller for creating and editing levels in a custom Unity Editor window.
/// </summary>
public class LEController : EditorWindow
{
    private string LevelEditorBasePrefabPath = "Assets/Prefabs/LevelEditor/LEBase.prefab";
    ...
    {
        GetWindow<LEController>("Level Editor");
    }
    ...
    private void ResetVariables()
    {
        ...
        rowCount = 1;
        columnCount = 1;
        Array.Clear(levelSquares, 0, levelSquares.Length);
    }
    ...
    private void GUILevelSelector()
    {
        ...
        string inputLevel = GUILayout.TextField(" " + levelNumber, new GUILayoutOption[] { GUILayout.Width(50) });

        if (int.TryParse(inputLevel, out int newLevel) && newLevel != levelNumber)
        {
            if(LoadDataFromLocal(newLevel))
        ...
        if (GUILayout.Button("New level", new GUILayoutOption[] { GUILayout.Width(100) }))
            AddLevel(saveLastLevel: true, newLevelNumber: GetLastLevel() + 1);
    ...
    private void RenderMoveCountEditor()
    {
        GUILayout.BeginHorizontal();
        GUILayout.Space(60);
    ...
    private void ShowItems()
    {
        ...
        if(LevelEditorBase.ItemModels.Count > 0)
        {
            for(int i=0; i < LevelEditorBase.ItemModels.Count; i++)
            {
                ...
                if(itemModel.itemIcons[0] != null)
                {
                    if (GUILayout.Button(itemModel.itemIcons[0].texture, new GUILayoutOption[] { GUILayout.Width(50), GUILayout.Height(50) }))
                    {
    ...
    private void ShowSelectedItemModelInfo()
    {
        ...
        for(int i=0; i < selectedItemModel.itemHealth; i++)
        {
            selectedItemModel.itemIcons[i] = (Sprite)EditorGUILayout.ObjectField
            (
                selectedItemModel.itemIcons[i],
                typeof(Sprite),
                false,
                GUILayout.Width(75),
                GUILayout.Height(75)
        ...
        EditorGUILayout.LabelField("Health:", new GUILayoutOption[] { GUILayout.Width (50) });
        int itemHealthBackup = selectedItemModel.itemHealth;
        selectedItemModel.itemHealth = EditorGUILayout.IntField(selectedItemModel.itemHealth, new GUILayoutOption[] { GUILayout.Width(50) });
        if(itemHealthBackup != selectedItemModel.itemHealth) 
        {
            SaveSpriteList();
            SaveItem();
        }
    ...
    private void SaveSpriteList()
    {
        selectedItemModel.itemIcons.Clear();

        for(int i=0; i < selectedItemModel.itemHealth; i++)
        {
            selectedItemModel.itemIcons.Add(null);
        }
    }
    ...
    #region GameField
    private void RenderGameFieldEditor()
    {
        GUILayout.BeginHorizontal();
        GUILayout.Space(60);
        GUILayout.BeginVertical();
        ...
        if(rowCount > 0 && columnCount > 0)
        {
            ...
            for(int i=0; i < rowCount; i++)
            {
                ...
                for(int j=0; j < columnCount; j++)
                {
                    ...
                    {
                        if(GUILayout.Button(LevelEditorBase.ItemModels[levelSquares[i * columnCount + j]].itemIcons[0].texture, new GUILayoutOption[] { GUILayout.Width(50), GUILayout.Height(50) }))
                        {
